...
const Admin = (() => {
  ...
  /*
   * getCategoryDisplayName — builds the label shown on post cards/badges.
   *
   * For a subcategory, this includes its parent project so the badge
   * reads "Sanctuary of Spirits › Bugs & Fixes" instead of just
   * "Bugs & Fixes" — useful once you're juggling several projects that
   * might each have their own "Bugs & Fixes" subcategory.
   * For a top-level project (or if the parent can't be found), it just
   ...
   * ============================================================
   *
   *  Categories are now two-level: PROJECTS (top-level) with
   *  SUBCATEGORIES nested underneath them. The sidebar shows this
   *  as an indented list — each project followed by its subcategories,
   *  slightly indented and in smaller text so the hierarchy reads
   *  clearly at a glance.
   */
   ...
   *
   * For each project, shows its own row PLUS a row for every
   * subcategory nested underneath it. A project's post count includes
   * posts filed directly under it AND posts filed under any of its
   * subcategories — so the number always reflects "everything related
   * to this project", not just posts with no subcategory chosen.
   *
   * Clicking a row sets the filter dropdown and refreshes the post grid.
   * A small ✕ button (visible on hover) lets you delete a category —
   * deleting a project also deletes its subcategories (see store.js).
   */
  function renderSidebarCategories() {
    const list  = $('categoryList');
    const posts = Store.Posts.getAll();
    const tops  = Store.Categories.getTopLevel(); // your projects

    list.innerHTML = ''; // clear and rebuild from scratch

    if (!tops.length) {
      list.innerHTML = '<li class="category-empty">No projects yet — add one below</li>';
      return;
    }

    tops.forEach(parent => {
      const children = Store.Categories.getChildren(parent.id);
      const childIds  = children.map(c => c.id);

      // Count posts filed directly under the project OR under any of its subcategories
      const totalCount = posts.filter(p => p.category === parent.id || childIds.includes(p.category)).length;
      list.appendChild(buildCategoryRow(parent, totalCount, false));

      // Render each subcategory indented underneath, with its own (exact-match) count
      children.forEach(child => {
        const childCount = posts.filter(p => p.category === child.id).length;
        list.appendChild(buildCategoryRow(child, childCount, true));
      });
    });
  }

  /*
   * buildCategoryRow — builds one <li> for either a project or a subcategory.
   *
   * PARAMETERS:
   * cat     — the category object (project or subcategory)
   * count   — how many posts to display next to it
   * isChild — true for a subcategory (adds indentation + smaller text)
   */
  function buildCategoryRow(cat, count, isChild) {
    const li = document.createElement('li');
    li.className = `category-item${isChild ? ' subcategory' : ' parent-category'}`;
    ...
    li.innerHTML = `
      ...
      <span class="category-row-actions">
        ...
        <button class="category-delete-btn" data-id="${cat.id}" aria-label="Delete ${cat.name}" title="Delete">✕</button>
      </span>`;

    // Clicking the row (but not the delete button) filters the post list
    li.addEventListener('click', e => {
      if (e.target.closest('.category-delete-btn')) return; // handled separately below
    ...
    // The ✕ delete button — stop the click from also triggering the row's filter click
    li.querySelector('.category-delete-btn').addEventListener('click', e => {
      e.stopPropagation();
      confirmDeleteCategory(cat.id, cat.name, !isChild);
    });
  ...
  /*
   * confirmDeleteCategory — shows a confirmation modal before deleting a category.
   *
   * If you're deleting a PROJECT that has subcategories underneath it,
   * the warning message tells you exactly how many will be deleted along
   * with it, since that's a cascade delete you can't undo.
   *
   * PARAMETERS:
   * id        — the category's ID
   * name      — the category's name (for the confirmation message)
  ...
  function confirmDeleteCategory(id, name, isProject) {
    ...
    showModal('Delete Category', `${cascadeWarning}Delete "${name}"? Posts in this category will become Uncategorized.`, [
   ...
   *
   * Each dropdown is now grouped by project using the HTML <optgroup>
   * element — browsers render this as a bold, non-selectable label
   * with its options indented underneath, which is exactly the visual
   * grouping we want without writing any custom dropdown component.
   *
   * The project itself is included as a selectable option inside its
   * own group (labelled "(general)") so you can file a post directly
   * under the project without picking a specific subcategory.
   */
  function populateCategorySelects() {
    const groupedOptions = buildGroupedCategoryOptions();
    const opts = '<option value="">Uncategorized</option>' + groupedOptions;
    ...
    if (f) f.innerHTML = '<option value="">all categories</option>' + groupedOptions;
  }

  /*
   * buildGroupedCategoryOptions — builds the shared <optgroup> HTML
   * used by every category dropdown in the admin panel.
   *
   * Returns one <optgroup> per project, containing:
   * 1. The project itself, as a "(general)" option
   * 2. Every subcategory nested under that project
   */
  function buildGroupedCategoryOptions() {
    const tops = Store.Categories.getTopLevel();
    return tops.map(parent => {
      const children = Store.Categories.getChildren(parent.id);
      const parentOption = `<option value="${parent.id}">${parent.name} (general)</option>`;
      const childOptions = children.map(c => `<option value="${c.id}">${c.name}</option>`).join('');
      return `<optgroup label="${parent.name}">${parentOption}${childOptions}</optgroup>`;
    }).join('');
  }
  ...
  function renderPosts() {
    ...
    grid.innerHTML = posts.map(post => {
      ...
      // Category badge (coloured pill label) — shows "Project › Subcategory" if applicable
      const badge = cat
        ? `<span class="category-badge" style="background:${cat.color}22;color:${cat.color};border:1px solid ${cat.color}44">${getCategoryDisplayName(cat)}</span>`
        : '';
  ...
  function savePost() {
    ...
    const tags     = $('postTags').value.split(',')   // split "bugfix, refactor, ai" by commas
   ...
   *
   * Lets you create either:
   * - A new PROJECT (a top-level category, e.g. "Sanctuary of Spirits")
   * - A new SUBCATEGORY nested under an existing project (e.g. "Bugs & Fixes")
   *
   * A toggle at the top switches between the two modes. Picking
   * "New Subcategory" reveals a dropdown to choose which project it
   * belongs to, and defaults the colour swatch to match that project's
   * colour (you can still pick a different one if you want).
   *
   * "New Subcategory" is disabled if you don't have any projects yet —
   * you need at least one project to nest a subcategory under.
   */
  function openAddCategoryModal() {
    const COLORS  = Store.Categories.getColors();
    const topLevelProjects = Store.Categories.getTopLevel();
    let picked = COLORS[0]; // default to the first colour (blue)
    let mode   = 'project'; // 'project' or 'subcategory'
    ...
    const parentOptions = topLevelProjects
      .map(p => `<option value="${p.id}" data-color="${p.color}">${p.name}</option>`)
      .join('');

    showModal('New Category', `
      <div class="modal-form-group">
        <label class="form-label">Type</label>
        <div class="tab-group modal-type-toggle" role="tablist">
          <button type="button" role="tab" class="tab-btn active" id="typeProjectBtn" aria-selected="true">New Project</button>
          <button type="button" role="tab" class="tab-btn" id="typeSubBtn" aria-selected="false" ${!topLevelProjects.length ? 'disabled' : ''}>New Subcategory</button>
        </div>
        ${!topLevelProjects.length ? '<p class="form-hint">Create a project first before adding subcategories.</p>' : ''}
      </div>
      <div class="modal-form-group hidden" id="parentPickerGroup">
        <label class="form-label" for="parentProject">Parent Project</label>
        <select id="parentProject" class="form-input">${parentOptions}</select>
      </div>
      <div class="modal-form-group">
        <label class="form-label" for="newCatName">Name</label>
        <input type="text" id="newCatName" class="form-input" placeholder="e.g. Sanctuary of Spirits" maxlength="30" />
      </div>
      <div class="modal-form-group">
    ...
    `, [
      ...
      { label: 'Create', cls: 'btn-primary', action: () => {
        ...
        const parentId = mode === 'subcategory' ? document.getElementById('parentProject')?.value : null;
        try {
          Store.Categories.create(name, picked, parentId);
          populateCategorySelects();
          renderSidebarCategories();
    ...
    // After the modal renders, wire up the type toggle, parent picker, and colour swatches
    setTimeout(() => {
      const typeProjectBtn = document.getElementById('typeProjectBtn');
      const typeSubBtn     = document.getElementById('typeSubBtn');
      const parentGroup    = document.getElementById('parentPickerGroup');
      const parentSelect   = document.getElementById('parentProject');

      // Selects a colour swatch to match the given hex colour (used when
      // switching to subcategory mode, to default to the parent's colour)
      function selectSwatchMatching(hexColor) {
        picked = hexColor;
        document.querySelectorAll('.color-swatch').forEach(s => {
          const match = s.dataset.color.toLowerCase() === hexColor.toLowerCase();
      ...
      typeProjectBtn?.addEventListener('click', () => {
        mode = 'project';
        typeProjectBtn.classList.add('active');    typeProjectBtn.setAttribute('aria-selected', 'true');
        typeSubBtn.classList.remove('active');     typeSubBtn.setAttribute('aria-selected', 'false');
        parentGroup.classList.add('hidden');
      });

      // Switch to "New Subcategory" mode
      typeSubBtn?.addEventListener('click', () => {
        if (typeSubBtn.disabled) return; // no projects exist yet — nothing to nest under
        mode = 'subcategory';
        typeSubBtn.classList.add('active');        typeSubBtn.setAttribute('aria-selected', 'true');
        typeProjectBtn.classList.remove('active'); typeProjectBtn.setAttribute('aria-selected', 'false');
        parentGroup.classList.remove('hidden');
        // Default the colour to match whichever project is currently selected
        const selectedOption = parentSelect?.selectedOptions[0];
        if (selectedOption) selectSwatchMatching(selectedOption.dataset.color);
      });

      // When you change which project a subcategory belongs to, update the default colour too
      parentSelect?.addEventListener('change', () => {
        const selectedOption = parentSelect.selectedOptions[0];
        if (selectedOption) selectSwatchMatching(selectedOption.dataset.color);
      });

      // Manual colour swatch selection (overrides the auto-matched default)
      document.querySelectorAll('.color-swatch').forEach(s => {
