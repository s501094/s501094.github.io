...
const AI = (() => {
 ...
 // const API_URL    = 'https://api.anthropic.com/v1/messages';
   ...
   *  "fixed a login bug today, took 2hrs to find it was a race
   *   condition, felt so good when it finally clicked. also
   *   cleaned up the sidebar CSS."
   *
  ...
  async function beautifyPost(rawContent, options = {}) {
     ...
     */
    const system = `You are a skilled technical writing assistant for a personal developer blog covering a range of projects (games, tools, apps, and everything in between).
...
- ${includeTips
    ...
    : 'Skip beginner tips.'}
- Do NOT invent project details that weren't in the raw notes
...
- Output ONLY the markdown, no preamble`;
     ...
     * about the project and category to help it write a better post.
     */
    const user = `Project: ${gameName || 'this project'}
...
Generate an appropriate title as the first H1 heading.`;
  ...
  async function beautifyDraft(content, title) {
    const system = `You are a blog editor. Polish this dev blog post draft:
...
- Output ONLY the improved markdown, no preamble or commentary`;
   ...
   */
  const DAILY_PROMPTS = [
    "What did you work on today? What went well, and what surprised you?",
    ...
    "Did you do any design, writing, or creative work today? What tools did you use?",
    "What's one thing you'd do differently if you were starting this project over?",
    "How does today's progress compare to your original plan?",
    "What inspired you to add something new today?",
    "Did you get any feedback, or test things yourself? What'd you notice?",
  ];
