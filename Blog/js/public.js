...
const Public = (() => {
   ...
   * "Bugs & Fixes" — useful once there are several projects that
   ...
   * ============================================================
   *
   *  Categories are two-level: PROJECTS (top-level) with SUBCATEGORIES
   *  nested underneath them. The sidebar shows each project followed
   *  by its subcategories, indented and in smaller text so visitors
   *  can tell at a glance which subcategory belongs to which project.
   *
   *  A project's post count includes posts filed directly under it
   *  AND posts filed under any of its subcategories — clicking a
   *  project shows "everything related to this project", while
   *  clicking a specific subcategory narrows down to just that.
   *
  ...
  function renderSidebarCategories() {
    ...
    const tops  = Store.Categories.getTopLevel(); // the projects
   ...
   * isChild — true for a subcategory (adds indentation + smaller text via CSS)
   */
   ...
   *
   * Grouped by project using the HTML <optgroup> element — browsers
   * render this as a bold, non-selectable label with its options
   * indented underneath, giving us the project/subcategory grouping
   * for free without a custom dropdown component.
   *
  ...
  function populateCategorySelect() {
    ...
    const groupedOptions = tops.map(parent => {
      const children = Store.Categories.getChildren(parent.id);
      // The ternary re-selects whichever option was previously chosen, after the list rebuilds
      const parentOption = `<option value="${parent.id}"${parent.id === curr ? ' selected' : ''}>${parent.name} (all)</option>`;
      const childOptions = children
        .map(c => `<option value="${c.id}"${c.id === curr ? ' selected' : ''}>${c.name}</option>`)
        .join('');
      return `<optgroup label="${parent.name}">${parentOption}${childOptions}</optgroup>`;
    }).join('');

    sel.innerHTML = '<option value="">all categories</option>' + groupedOptions;
  }
  ...
  function renderPosts() {
    ...
    grid.innerHTML = posts.map(post => {
      ...
      // Shows "Project › Subcategory" if the post is filed under a subcategory
  ...
  function openPost(id) {
    ...
    // Build the category badge — shows "Project › Subcategory" if applicable
    const cat = getCategoryById(post.category);
    const badgeHtml = cat
      ? `<span class="category-badge" style="background:${cat.color}22;color:${cat.color};border:1px solid ${cat.color}44">${getCategoryDisplayName(cat)}</span>`
      : '';
